...
def create_model_processors(accelerator, config, model_dtype):
    ...
    return flux_rm, vae_processor, noise_scheduler
...
def main():
    ...
    flux_rm, vae_processor, noise_scheduler = create_model_processors(accelerator, config, model_dtype)
    ...
    # Create optimizer
    trainable_params = list(filter(lambda p: p.requires_grad, flux_rm.parameters()))
    ...
    flux_rm.backbone, flux_rm.reward_head, optimizer, train_dataloader = accelerator.prepare(
        flux_rm.backbone, flux_rm.reward_head, optimizer, train_dataloader
    )
    # import pdb; pdb.set_trace()
    ...
    def count_params(model, trainable_only=False):
        ...
        return total
    print("Total model params: {:.2f} M".format(count_params(flux_rm) / 1e6))
    # print("Trainable model params: {:.2f} M".format(count_params(flux_rm, trainable_only=True) / 1e6))
    # print("backbone params: {:.2f} M".format(count_params(flux_rm.backbone) / 1e6))
    # print("reward head params: {:.2f} M".format(count_params(flux_rm.reward_head) / 1e6))
    # import pdb; pdb.set_trace()
    # flux_rm.backbone = accelerator.prepare(flux_rm.backbone)
    # flux_rm.reward_head = accelerator.prepare(flux_rm.reward_head)
