...
from diffusion_rm.models.zimage_rm import ZImageRewardModel
...
def create_model_processors(accelerator, config, model_dtype):
    ...
    return zimage_rm, vae_processor, noise_scheduler
...
def main():
    ...
    zimage_rm, vae_processor, noise_scheduler = create_model_processors(accelerator, config, model_dtype)
    ...
    # Create optimizer
    trainable_params = list(filter(lambda p: p.requires_grad, zimage_rm.parameters()))
    ...
    zimage_rm.backbone, zimage_rm.reward_head, optimizer, train_dataloader = accelerator.prepare(
        zimage_rm.backbone, zimage_rm.reward_head, optimizer, train_dataloader
    )
    # import pdb; pdb.set_trace()
