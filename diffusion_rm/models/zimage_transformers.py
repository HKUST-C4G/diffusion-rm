## COPIED FROM https://github.com/Tongyi-MAI/Z-Image/blob/main/src/zimage/transformer.py
...
class ZImageBackbone(ZImageTransformer2DModel):
    def __init__(self, transformer, config_model):
        ...
        for name, module in transformer.named_children():
            ...
            if name == 'layers':
                self.layers = nn.ModuleList(
                    module[:config_model.num_transformer_layers]
                )
            else:
                setattr(self, name, module)

        excluded_keys = {'_parameters', '_modules', '_buffers', '_backward_hooks', '_forward_hooks', '_forward_pre_hooks', '_state_dict_hooks', '_load_state_dict_pre_hooks'}
        for name, value in transformer.__dict__.items():
            if name not in excluded_keys and not name.startswith('__'):
    ...
    def forward(
        ...
        for index_layer, layer in enumerate(self.layers):
            ...
            if index_layer + 1 in self.visual_head_idx:
                visual_features = [unified[i][:x_item_seqlens[i]] for i in range(bsz)]
                visual_features = pad_sequence(visual_features, batch_first=True, padding_value=0.0)
                hidden_states_list.append(visual_features)
            if index_layer + 1 in self.text_head_idx:
                text_features = [unified[i][x_item_seqlens[i]:] for i in range(bsz)]
                text_features = pad_sequence(text_features, batch_first=True, padding_value=0.0)
                encoder_hidden_states_list.append(text_features)

        # unified = self.all_final_layer[f"{patch_size}-{f_patch_size}"](unified, adaln_input)
        # unified = list(unified.unbind(dim=0))
        # x = self.unpatchify(unified, x_size, patch_size, f_patch_size)

        return adaln_input, hidden_states_list, encoder_hidden_states_list
