...
def pickscore_score(device):
    from flow_grpo.pickscore_scorer import PickScoreScorer, PickScoreScorerWithGrad

    # scorer = PickScoreScorer(dtype=torch.float32, device=device)
    scorer = PickScoreScorerWithGrad(dtype=torch.float32, device=device)
    ...
    def _fn(images, prompts, metadata):
        if isinstance(images, torch.Tensor):
            ...
            raise ValueError("images should be a torch.Tensor")
...
def maniqa_score(device):
    from flow_grpo.maniqa_scorer import MANIQAScorer
    
    scorer = MANIQAScorer(device=device, dtype=torch.float32, model_name="maniqa-pipal")
...
def diffusion_rm_score(pipeline, device, dtype):
    ...
    def _fn(latents, prompt_embeds, pooled_prompt_embeds, u=0.4, metadata=None):
        ...
        scores = (rewards.float() + 12.0 ) / 10.0
        score_details['diffusion_rm_0.4'] = scores
        total_scores = [score for score in scores]
