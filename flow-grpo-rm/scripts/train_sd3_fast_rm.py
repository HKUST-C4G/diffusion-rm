...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, proxy_reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters):
    ...
    for test_batch in tqdm(
        ...
        with autocast():
            with torch.no_grad():
                images, last_latents, _, _, _ = pipeline_with_logprob(
                    pipeline,
...
def main(_):
    ...
    ## IMPORTANT: make sure reward fn is created before we create lora model
    ...
    while True:
        ...
        for i in tqdm(
            ...
            with autocast():
                with torch.no_grad():
                    images, last_latents, latents, log_probs, timesteps = pipeline_with_logprob(
                        pipeline,
                        prompt_embeds=prompt_embeds,
                        pooled_prompt_embeds=pooled_prompt_embeds,
                        negative_prompt_embeds=sample_neg_prompt_embeds,
        ...
        for sample in tqdm(
            ...
            with torch.no_grad():
                ...
                proxy_rewards, proxy_reward_metadata = sample["proxy_rewards"].result()
            # accelerator.print(reward_metadata)
            sample["rewards"] = {
                key: torch.as_tensor(value, device=accelerator.device).float()
                for key, value in rewards.items()
            }
            sample['proxy_rewards'] = {
                key: torch.as_tensor(value, device=accelerator.device).float()
                for key, value in proxy_rewards.items()
            }
        ...
        if epoch % 10 == 0 and accelerator.is_main_process:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                sampled_proxy_rewards = [proxy_rewards['avg'][i] for i in sample_indices]
                # import pdb; pdb.set_trace()
                accelerator.log(
                    {
                        "images": [
                            wandb.Image(
                                ...
                                caption=f"{prompt:.100} | avg: {avg_reward.item():.2f} | proxy_avg: {proxy_avg_reward.item():.2f}",
                            )
                            for idx, (prompt, avg_reward, proxy_avg_reward) in enumerate(zip(sampled_prompts, sampled_rewards, sampled_proxy_rewards))
                        ],
                    },
                    step=global_step,
                )
        ...
        samples['rewards']['proxy_avg'] = samples['proxy_rewards']['avg'].unsqueeze(1).repeat(1, num_train_timesteps)
        del samples['proxy_rewards']
        ...
        if config.per_prompt_stat_tracking:
            ...
            ## for proxy
            advantages_proxy = stat_tracker.update(prompts, gathered_rewards['proxy_avg'])
            stat_tracker.clear()

            advantages = advantages + 0.2 * advantages_proxy

            zero_std_ratio, reward_std_mean = calculate_zero_std_ratio(prompts, gathered_rewards)
            
            proxy_reward_std_mean = gathered_rewards['proxy_avg'].std()
                ...
                accelerator.log(
                    {
                        ...
                        "proxy_reward_std_mean": proxy_reward_std_mean,
                    },
                    step=global_step,
                )
        else:
