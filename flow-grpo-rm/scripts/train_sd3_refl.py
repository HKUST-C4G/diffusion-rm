...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, proxy_reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters):
    ...
    for test_batch in tqdm(
        ...
        for idx, i in enumerate(range(len(images))):
            ...
            caption = f"GPU:{proc_id} | {prompts[i]} | {rewards['avg'][i].item():.2f} | Proxy: {proxy_rewards['avg'][i].item():.2f}"
    ...
    while True:
        ...
        with accelerator.accumulate(pipeline.transformer):
            with autocast():
                ...
                pred_original_sample = (pred_original_sample / pipeline.vae.config.scaling_factor) + pipeline.vae.config.shift_factor
                ...
                proxy_rewards = executor.submit(proxy_reward_fn, images, prompts, prompt_metadata, only_strict=True)
                # yield to to make sure reward computation starts
                time.sleep(0)
