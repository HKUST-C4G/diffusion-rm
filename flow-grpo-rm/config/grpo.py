...
def pickscore_sd3_fast_rm():
    ...
    config.save_freq = 30 # epoch
    config.eval_freq = 30
    config.run_name = 'GRPO-Fast-nocfg-OurRM-0.2_proxy_adv_clip3e-6_lr5e-5-update'
    config.save_dir = f'logs/{config.run_name}'
    config.save_eval_dir = f'log_save_images/{config.run_name}'
    config.reward_fn = {
        "pickscore": 1.0,   # ignore it
    }
...
def pickscore_sd3_fast_rm_debug():
    gpu_number=1
    config = compressibility()
    config.dataset = os.path.join(os.getcwd(), "dataset/pickscore")
